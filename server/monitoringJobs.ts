...
import {
  notifyLendersOfCovenantBreach,
  notifyLendersOfContractRenewal,
} from "./lenderNotifications";
    ...
    for (const project of allProjects) {
      ...
      if (tier1Coverage < tier1Target * 0.9) {
        ...
        await db.insert(covenantBreachEvents).values({
          ...
          impactAssessment:
            "High impact: Tier 1 covenant breach may trigger lender review and affect project financing terms.",
  ...
  try {
    ...
    for (const agreement of allAgreements) {
      ...
      if (endDate <= ninetyDaysFromNow && endDate > today) {
        ...
        const hasRecentAlert = existingAlerts.some((e: any) =>
          e.narrativeExplanation?.includes(agreement.id.toString())
        );
